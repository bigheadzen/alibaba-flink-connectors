...
public final class RetryUtil {
	...
	private static class Retry {
		...
		public <T> T doRetry(Callable<T> callable, int retryTimes, long sleepTimeInMilliSecond, boolean exponential, List<Class<?>> retryExceptionClasss)
			...
			if (null == callable) {
				throw new IllegalArgumentException("Invalid parameter, callable is null.");
			}

			if (retryTimes < 1) {
				throw new IllegalArgumentException("Invalid parameter, retryTimes less than 1.");
			}
			...
			for (int i = 0; i < retryTimes; i++) {
				try {
					...
					saveException = e;
					if (i == 0) {
						LOG.error("First error on retry: %s", saveException.getMessage(), saveException);
					}
					...
					if (i + 1 < retryTimes && sleepTimeInMilliSecond > 0) {
						...
						LOG.error("Will retry after sleep. current retry: {}, retryTimes: {}, sleep: {}, realTimeSleep: {}, error: {}",
								i + 1, retryTimes, timeToSleep, realTimeSleep, e.getMessage());
					}
				}
			}
			throw saveException;
